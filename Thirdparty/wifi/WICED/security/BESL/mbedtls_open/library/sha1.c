...
void sha1_hmac_starts(sha1_hmac_context *ctx, const unsigned char *key, uint32_t keylen)
{
    ...
    mbedtls_sha1_update(&ctx->ctx, ctx->ipad, 64);
...
{
    mbedtls_sha1_update(&ctx->ctx, input, ilen);
}
...
void sha1_hmac_finish(sha1_hmac_context * ctx, unsigned char output[20])
{
    ...
    mbedtls_sha1_finish(&ctx->ctx, tmpbuf);
    mbedtls_sha1_starts(&ctx->ctx);
    mbedtls_sha1_update(&ctx->ctx, ctx->opad, 64);
    mbedtls_sha1_update(&ctx->ctx, tmpbuf, 20);
    mbedtls_sha1_finish(&ctx->ctx, output);
           ...
           unsigned char output[20])
{
   ...
   sha1_hmac_finish(&ctx, output);
    ...
    memset(&ctx, 0, sizeof(sha1_hmac_context));
}
#endif /* MBEDTLS_SHA1_ALT */
/* WICED_MBEDTLS End */
