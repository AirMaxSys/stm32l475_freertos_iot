...
const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_default =
{
    ...
    0xFFFFFFF, /* Any curve     */
    /* WICED_MBEDTLS Start */
    /* FIXME: Changing default RSA key size from 2048 to 1024 as some servers still
     * use (e.g. test.mosquitto.org) RSA keys of 1024 bits. In future,
     * this field should be made user-configurable from 'config.h'.
     */
    /* 2048, */
    1024,
    /* WICED_MBEDTLS End */
...
int mbedtls_x509_crt_parse( mbedtls_x509_crt *chain, const unsigned char *buf, size_t buflen )
{
    ...
    if( buflen != 0 &&
        strnstrn( (const char *) buf, buflen , "-----BEGIN CERTIFICATE-----", strlen( "-----BEGIN CERTIFICATE-----" ) ) != NULL &&
        strnstrn( (const char *) buf, buflen,  "-----END CERTIFICATE-----", strlen( "-----END CERTIFICATE-----" ) ) != NULL )
    ...
    if( buf_format == MBEDTLS_X509_FORMAT_PEM )
    {
        ...
        /* WICED_MBEDTLS End */
        {
            size_t use_len;
            /* WICED_MBEDTLS Start */
            if( strnstrn( (const char *) buf, buflen , "-----BEGIN CERTIFICATE-----", strlen( "-----BEGIN CERTIFICATE-----" ) ) == NULL &&
                strnstrn( (const char *) buf, buflen,  "-----END CERTIFICATE-----", strlen( "-----END CERTIFICATE-----" ) ) == NULL )
            {
...
/* commented out unused code which used only for printing certificate info */
/* WICED_MBEDTLS Start */
