    ...
    *olen = 0;
    ...
    if( ctx->cipher_info->mode == MBEDTLS_MODE_CBC )
    {
        ...
        /* if( ( ctx->operation == MBEDTLS_DECRYPT && NULL != ctx->add_padding &&
                     ...
                     ( ctx->operation == MBEDTLS_ENCRYPT && NULL != ctx->add_padding &&
                     ilen + ctx->unprocessed_len < block_size ) ) */
        if( ( ctx->operation == MBEDTLS_DECRYPT && NULL != ctx->add_padding &&
        ...
        if( 0 != ilen )
        {
            ...
            /* coverity-34199 [Divison or modulo by zero]
                  FALSE-POSITIVE: mbedtls_cipher_get_block_size function can return 0 if either ctx or ctx->cipher is NULL. ctx and ctx->cipher if NULL then in the start of the function above
                  itself it will return. So before calling mbedtls_cipher_get_block_size ctx and ctx->cipher will not be NULL. */
